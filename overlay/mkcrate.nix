  ...
  buildCmd =
    ...
    in
      if compileMode != "doctest" then ''
        ${rustToolchain}/bin/cargo build $CARGO_VERBOSE ${optionalString release "--release"} ${lib.strings.concatStringsSep " " cargoBuildFlags} --target ${rustHostTriple} ${buildMode} \
